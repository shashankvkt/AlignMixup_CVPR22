...
from models.wrn16x8_classifier import WideResNet_classifier
...
parser = argparse.ArgumentParser(description='Trains ResNet on CIFAR-10/100', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser.add_argument('--network', type=str, default='resnet', choices=['resnet', 'wideresnet'], help='Choose between resnet18/WRN16x8')
parser.add_argument('--dataset', type=str, default='cifar10', choices=['cifar10', 'cifar100'], help='Choose between Cifar10/100')
parser.add_argument('--data_dir', type = str, default = '/srv/tempdd/svenkata/Datasets/data',
                        help='path to train/test folder')
parser.add_argument('--save_dir', type = str, default = '/nfs/pyrex/raid6/svenkata/weights/AlignMixup_CVPR22/cifar100/',
...
if not os.path.exists(args.save_dir + args.network):
	os.makedirs(args.save_dir + args.network)
...
if (args.dataset == 'cifar10'):
	train_data = datasets.CIFAR10(root=args.data_dir, train=True, download=True, transform=transform_train)
	test_data = datasets.CIFAR10(root=args.data_dir, train=False, download=True, transform=transform_test)

elif(args.dataset == 'cifar100'):
	train_data = datasets.CIFAR100(root=args.data_dir, train=True, download=True, transform=transform_train)
	test_data = datasets.CIFAR100(root=args.data_dir, train=False, download=True, transform=transform_test)
...
if args.network == 'resnet':
	...
	model = WideResNet_classifier(args.num_classes)
